...
/**
 * The persistent class for the CMN_INTERFACE_DATA database table.
 * 
 */
@Entity
@Table(name="CMN_INTERFACE_DATA")
@NamedQuery(name="CmnInterfaceData.findAll", query="SELECT c FROM CmnInterfaceData c")
public class CmnInterfaceData implements Serializable {
	

	@EmbeddedId
	private CmnInterfaceDataPK id;
	...
	@ManyToOne
	@JoinColumns({
		@JoinColumn(name="IF_FILE_ID", referencedColumnName="IF_FILE_ID", insertable=false, updatable=false),
		@JoinColumn(name="SEQ_NO", referencedColumnName="SEQ_NO", insertable=false, updatable=false)
		})
	private CmnFileHdr cmnFileHdr;

	public CmnInterfaceData() {
	}

	public CmnInterfaceDataPK getId() {
		return this.id;
	}

	public void setId(CmnInterfaceDataPK id) {
		this.id = id;
	}
	...
	public CmnFileHdr getCmnFileHdr() {
		return this.cmnFileHdr;
	}

	public void setCmnFileHdr(CmnFileHdr cmnFileHdr) {
		this.cmnFileHdr = cmnFileHdr;
	}
