 ...
 */
public class B000040Writer implements
		ItemWriter<B000040OrdrDtlsOutputBean> {
	/* Constant LOG */
	private static final Log LOG = LogFactory
			.getLog(B000040Writer.class.getName());
	/**
	...
	public void beforeStep(StepExecution stepExecution)
		...
		String insertORderDetailsTmp = B000040QueryConstants.insertOrderDetailsTmp.toString();
		...
		try {
			LOG.info("Prepared Query********* " + insertORderDetailsTmp);
			connection = connectionProvider.getConnection();
			preparedStmt = connection.prepareStatement(insertORderDetailsTmp);			
	...
	public void addRecord(B000040OrdrDtlsOutputBean item) {
		try {
			preparedStmt.setString(1, item.getPorCd());		
			
			preparedStmt.setString(2, item.getProdMnth());
			
			preparedStmt.setString(3, item.getProdWkNo());	
			preparedStmt.setString(4, item.getOseiId());	
			preparedStmt.setString(5, item.getPotCd());	
			preparedStmt.setString(6, (item.getProdOrdrNo() != null) ? item.getProdOrdrNo() : "1" );//TODO : change prodOrderNo 	
			preparedStmt.setString(7, item.getOfflnPlanDate());	
			preparedStmt.setString(8, item.getProdPlntCd());	
			preparedStmt.setString(9, item.getLineClass());	
			preparedStmt.setString(10, item.getProdDayNo());	
			preparedStmt.setString(11, queryInputVO.getOrdrTakBseMnth());
			preparedStmt.setString(12, queryInputVO.getOrdrTakBseWkNo());
