 ...
 */
public class B000040AttachServicePrmtrWriter implements
		ItemWriter<B000040OrdrDtlsOutputBean> {
	/* Constant LOG */
	private static final Log LOG = LogFactory
			.getLog(B000040AttachServicePrmtrWriter.class.getName());
	/**
	...
	public void beforeStep(StepExecution stepExecution)
		...
		String updtOrdrDtilsTmp = B000040QueryConstants.updateSrvcePrmtr.toString();
	...
	public void addRecord(B000040OrdrDtlsOutputBean item) {
		try {
			...
			Object[] srvcPrmtrDtls = queryDataService.extractSrvcPrmtrDtls(
					item.getPorCd(), item.getProdFmyCd(), item.getBuyerCd(), item.getProdMnth(),item.getEndItem(),
					item.getSpecDestnCd());
			
			preparedStmt.setString(1, srvcPrmtrDtls[0].toString()); // DealerLst
			preparedStmt.setString(2, srvcPrmtrDtls[1].toString());	// Owner Manual
			preparedStmt.setString(3, srvcPrmtrDtls[2].toString());	//Warranty Booklet
			preparedStmt.setString(4, item.getPorCd());
			preparedStmt.setString(5, item.getOseiId());
