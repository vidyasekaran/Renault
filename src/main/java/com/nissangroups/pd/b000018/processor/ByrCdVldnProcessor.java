 ...
 * Process Outline :Validate teh Buyer Code for the Corresponding POR from Monthly Order Interface TRN with Master Tables. 
 *
 * <Revision History>
...
public class ByrCdVldnProcessor implements
		ItemProcessor<InputBean, InputBean> {

	/** Constant LOG. */
	private static final Log LOG = LogFactory.getLog
			(ByrCdVldnProcessor.class);
	...
	public InputBean process(InputBean input) throws PdApplicationException {
		...
		LOG.info(STEP_AFTER_START);
			...
			List<Integer> rowNo = vldnRepositoryObj.fetchInVldByrCd(input.getPorCd(),input.getFileId(),input.getTableName(),input.getSeqNo());
			if(rowNo!=null && !(rowNo.isEmpty())){
				String errorMessage = M00059.replaceAll("&1", "Buyer Code");
