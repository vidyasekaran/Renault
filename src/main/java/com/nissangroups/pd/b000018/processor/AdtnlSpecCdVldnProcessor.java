...
public class AdtnlSpecCdVldnProcessor implements
	...
	public InputBean process(InputBean input) throws PdApplicationException {
			...
			List<Integer> rowNo =  vldnRepositoryObj.fetchInVldAddtnlSpecCd(input.getPorCd(),input.getFileId(),input.getTableName(),input.getSeqNo());
			if(rowNo!=null && !(rowNo.isEmpty())){
				String errorMessage = M00059.replaceAll("&1", "Additional Spec Code");
