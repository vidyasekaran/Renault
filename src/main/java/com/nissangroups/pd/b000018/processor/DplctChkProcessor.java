...
public class DplctChkProcessor implements
		ItemProcessor<InputBean, InputBean> {

	/** Constant LOG. */
	private static final Log LOG = LogFactory.getLog
			(DplctChkProcessor.class);
	...
	public InputBean process(InputBean input) throws PdApplicationException {
		...
		LOG.info(STEP_AFTER_START);
			...
			List<Integer> rowNo =  vldnRepositoryObj.fetchDuplicateRcrds(input.getPorCd(),input.getFileId(),input.getTableName(),input.getSeqNo());
			if(rowNo!=null && !(rowNo.isEmpty())){	
				String errorMessage = M00058;
