 ...
 * Process Outline :Validate the Spec Destination Cd for the 
 * corresponding Por Cd,EI,Additional Spec Cd,Buyer Cd from Monthly Order Interface TRN with Master Tables. 
 *
 * <Revision History>
...
public class SpecDestCdVldnProcessor implements
		ItemProcessor<InputBean, InputBean> {

	/** Constant LOG. */
	private static final Log LOG = LogFactory.getLog
			(SpecDestCdVldnProcessor.class);
	...
	public InputBean process(InputBean input) throws PdApplicationException {
		...
		LOG.info(STEP_AFTER_START);
			...
			List<Integer> rowNo =  vldnRepositoryObj.fetchInVldSpecDestCd(input.getPorCd(),input.getFileId(),input.getTableName(),input.getSeqNo());
			if(rowNo!=null && !(rowNo.isEmpty())){
				String errorMessage = M00059.replaceAll("&1", "Spec Destination Cd");
