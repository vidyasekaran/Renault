 ...
 * Process Outline :Validate the Production Period Type from Monthly Order Interface TRN with Master Tables. 
 *
 * <Revision History>
...
import static com.nissangroups.pd.util.PDMessageConsants.M00208;
...
public class PrdPeriodTypVldnProcessor implements
		ItemProcessor<InputBean, InputBean> {

	/** Constant LOG. */
	private static final Log LOG = LogFactory.getLog
			(PrdPeriodTypVldnProcessor.class);
	...
	public InputBean process(InputBean input) throws PdApplicationException {
		...
		LOG.info(STEP_AFTER_START);
			...
			List<Integer> rowNo =  vldnRepositoryObj.fetchInVldPrdPeriodTyp(input.getPorCd(),input.getFileId(),input.getTableName(),input.getSeqNo());
			if(rowNo!=null && !(rowNo.isEmpty())){	
				String errorMessage = M00208;
