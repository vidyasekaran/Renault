 ...
 * Process Outline :Validate the Order Take Base Period Type  from Monthly Order Interface TRN with Master Tables. 
 *
 * <Revision History>
...
import static com.nissangroups.pd.util.PDMessageConsants.M00069;
...
public class OrdrTkBsPeriodTypVldnProcessor implements
		ItemProcessor<InputBean, InputBean> {

	/** Constant LOG. */
	private static final Log LOG = LogFactory.getLog
			(OrdrTkBsPeriodTypVldnProcessor.class);
	...
	public InputBean process(InputBean input) throws PdApplicationException {
		...
		LOG.info(STEP_AFTER_START);
			...
			List<Integer> rowNo =  vldnRepositoryObj.fetchInVldOrdrTkBsPrdTyp(input.getPorCd(),input.getFileId(),input.getTableName(),input.getSeqNo());
			if(rowNo!=null && !(rowNo.isEmpty())){	
				String errorMessage = M00069;
