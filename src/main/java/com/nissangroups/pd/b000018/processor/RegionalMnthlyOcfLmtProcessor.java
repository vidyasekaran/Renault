...
import com.nissangroups.pd.repository.RegionalMnthlyOcfLimitTrnRepository;
import com.nissangroups.pd.util.CommonUtil;



public class RegionalMnthlyOcfLmtProcessor implements
		ItemProcessor<InputBean, InputBean> {

	/** Constant LOG. */
	private static final Log LOG = LogFactory.getLog
			(RegionalMnthlyOcfLmtProcessor.class);

	@Autowired(required=false)
	private RegionalMnthlyOcfLimitTrnRepository rgnlMnthlyOcfLmtRepositoryObj;
	...
	public InputBean process(InputBean input) throws PdApplicationException, ParseException {
		...
		rgnlMnthlyOcfLmtRepositoryObj.initialisation(input,distinctByrGrpCarSrsList);
		...
		for(Object[] obj : distinctByrGrpCarSrsList){
			...
			rgnlMnthlyOcfLmtRepositoryObj.updateNewOrdrDtls(input.getPorCd(), prdMnth, ordrTkBsMnth,carSrs,byrGrpCd,ocfByrGrpCd,ocfRegionCd);
