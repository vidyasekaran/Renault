...
/**
* The Class B000047RmngOrdrsReportReader read the data from B000047 error report class and process .
* 
* @author z016127
*/
public class B000047RmngOrdrsReportReader implements ItemReader<B000047ReportDao> {
		...
		private static final Log LOG = LogFactory.getLog
				(B000047RmngOrdrsReportReader.class);
		...
		public B000047ReportDao read() throws Exception {
			...
			LOG.info(REPORT_SIZE +b000047ReportDao.getRmngOrdrsReportList().size());
			if(currentIndex < b000047ReportDao.getRmngOrdrsReportList().size()){
				LOG.info(DOLLAR + OUTSIDE_METHOD + DOLLAR);
				return b000047ReportDao.getRmngOrdrsReportList().get(currentIndex++);
			}
			else{
			LOG.info(DOLLAR + OUTSIDE_METHOD + DOLLAR);
			return null;
